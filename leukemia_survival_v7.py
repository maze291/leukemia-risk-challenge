...
# # Step 1: Colab setup
#
# This notebook builds V7 leakage-safe survival-risk submissions for the
# leukemia challenge. V7 is a controlled AML/MDS domain-feature ablation study
# on top of the V6 feature set.
#
# Long-running cells:
...
SAVE_V7_SUBMISSIONS = env_flag("SAVE_V7_SUBMISSIONS", False)

PHASE_ORDER = ["v6", "v7a", "v7b", "v7c", "v7d", "v7e"]
...
v7_blocks_train = {
    "v7a": make_v7a_cytogenetic_features(clinical_train),
    "v7b": make_v7b_molecular_features(molecular_train, clinical_train["ID"]),
    "v7c": make_v7c_mr_helper_features(molecular_train, clinical_train["ID"]),
    "v7d": make_v7d_molecular_proxy_features(molecular_train, clinical_train["ID"]),
    "v7e": make_v7e_clinical_features(clinical_train),
}
v7_blocks_test = {
    "v7a": make_v7a_cytogenetic_features(clinical_test),
    "v7b": make_v7b_molecular_features(molecular_test, clinical_test["ID"]),
    "v7c": make_v7c_mr_helper_features(molecular_test, clinical_test["ID"]),
    "v7d": make_v7d_molecular_proxy_features(molecular_test, clinical_test["ID"]),
    "v7e": make_v7e_clinical_features(clinical_test),
}


def build_full_feature_frame_for_phase(phase, split):
    if split == "train":
        f = feature_train_v6.copy()
        blocks = v7_blocks_train
    elif split == "test":
        f = feature_test_v6.copy()
        blocks = v7_blocks_test
    else:
        raise ValueError(f"Unknown split {split!r}")

    if phase not in PHASE_ORDER:
        raise ValueError(f"Unknown phase {phase!r}")
    if PHASE_ORDER.index(phase) >= PHASE_ORDER.index("v7a"):
        f = pd.concat([f, blocks["v7a"]], axis=1)
    if PHASE_ORDER.index(phase) >= PHASE_ORDER.index("v7b"):
        f = pd.concat([f, blocks["v7b"]], axis=1)
    if PHASE_ORDER.index(phase) >= PHASE_ORDER.index("v7c"):
        f = pd.concat([f, blocks["v7c"]], axis=1)
        f = pd.concat([f, make_v7c_interaction_features(f)], axis=1)
    if PHASE_ORDER.index(phase) >= PHASE_ORDER.index("v7d"):
        f = pd.concat([f, blocks["v7d"]], axis=1)
    if PHASE_ORDER.index(phase) >= PHASE_ORDER.index("v7e"):
        f = pd.concat([f, blocks["v7e"]], axis=1)

    f = f.apply(pd.to_numeric, errors="coerce")
    f = f.replace([np.inf, -np.inf], np.nan)
    if f.columns.duplicated().any():
        duplicates = f.columns[f.columns.duplicated()].tolist()
        raise ValueError(f"Duplicate raw feature names in {phase}/{split}: {duplicates[:10]}")
    return f


def build_features_for_phase(phase):
    feature_train_phase = build_full_feature_frame_for_phase(phase, "train")
    feature_test_phase = build_full_feature_frame_for_phase(phase, "test").reindex(columns=feature_train_phase.columns)
    X_train_phase = feature_train_phase.loc[train_ids].copy()
    X_test_phase = feature_test_phase.loc[test_ids].copy()

    assert "CENTER" not in X_train_phase.columns
    assert "CENTER" not in X_test_phase.columns
    assert "CYTOGENETICS" not in X_train_phase.columns
    assert "CYTOGENETICS" not in X_test_phase.columns
    assert list(X_test_phase.index) == list(random_submission["ID"])
    assert list(X_train_phase.columns) == list(X_test_phase.columns)
    assert not X_train_phase.columns.duplicated().any()
    assert not X_test_phase.columns.duplicated().any()
    return X_train_phase, X_test_phase


PHASE_MATRICES = {phase: build_features_for_phase(phase) for phase in PHASE_ORDER}
feature_count_rows = []
for phase, (X_tr_phase, X_te_phase) in PHASE_MATRICES.items():
    feature_count_rows.append(
        {
            "phase": phase,
            "train_rows": X_tr_phase.shape[0],
            "train_features": X_tr_phase.shape[1],
            "test_rows": X_te_phase.shape[0],
            "test_features": X_te_phase.shape[1],
            "train_missing_pct": round(float(X_tr_phase.isna().mean().mean() * 100), 4),
        }
    )
feature_counts_by_phase = pd.DataFrame(feature_count_rows)
display(feature_counts_by_phase)
feature_counts_by_phase.to_csv(OUTPUT_DIR / "v7_feature_counts_by_phase.csv", index=False)

X_train_raw, X_test_raw = PHASE_MATRICES["v6"]
print("Default raw matrix for helper initialization is V6:", X_train_raw.shape, X_test_raw.shape)

if FEATURE_COUNTS_ONLY:
    print("FEATURE_COUNTS_ONLY=1: feature builders completed; skipping full V7 model run.")
    sys.exit(0)
...
# %% [markdown]
# # Step 14: V7 GroupKFold phase comparison harness
#
# V7 model and ensemble selection uses GroupKFold by CENTER only. Random KFold,
# LOCO, KYW diagnostics, prior submissions, and leaderboard feedback are not
# used for feature acceptance, model selection, or final submission selection.
...
def evaluate_phase_ensembles(phase_name, group_results):
    ...
    for candidate_name, weights in candidates:
        ...
        ensemble_oof = np.zeros(len(PHASE_MATRICES[phase_name][0]), dtype=float)
...
def score_subset_ipcw(train_idx, valid_idx, pred):
    if len(valid_idx) < 20:
        return np.nan
    try:
        return ipcw_score(y[train_idx], y[valid_idx], pred[valid_idx])
    except Exception:
        return np.nan


def phase_delta_rows(phase_name, phase_oof, v6_oof, splits):
    rows = []
    centers = np.asarray(groups_center)
    for fold, (train_idx, valid_idx) in enumerate(splits, start=1):
        phase_fold_score = score_subset_ipcw(train_idx, valid_idx, phase_oof)
        v6_fold_score = score_subset_ipcw(train_idx, valid_idx, v6_oof)
        rows.append(
            {
                "phase": phase_name,
                "fold": fold,
                ...
                "v6_ipcw": v6_fold_score,
                "phase_ipcw": phase_fold_score,
                "delta_ipcw": phase_fold_score - v6_fold_score,
            }
        )
        for center in sorted(pd.Series(centers[valid_idx]).dropna().unique()):
            center_idx = valid_idx[centers[valid_idx] == center]
            phase_center_score = score_subset_ipcw(train_idx, center_idx, phase_oof)
            v6_center_score = score_subset_ipcw(train_idx, center_idx, v6_oof)
            rows.append(
                {
                    "phase": phase_name,
                    "fold": fold,
                    ...
                    "v6_ipcw": v6_center_score,
                    "phase_ipcw": phase_center_score,
                    "delta_ipcw": phase_center_score - v6_center_score,
                }
            )
    return rows


def acceptance_stats(phase_name, selected, v6_selected, delta_df):
    phase_score = selected["group_oof_ipcw"]
    v6_score = v6_selected["group_oof_ipcw"]
    scorable_centers = delta_df[(delta_df["phase"] == phase_name) & (delta_df["center"] != "__FOLD_ALL__")].dropna(subset=["delta_ipcw"])
    ...
    return {
        ...
        "selected_group_oof_ipcw": phase_score,
        "v6_selected_group_oof_ipcw": v6_score,
        "delta_vs_v6": phase_score - v6_score,
...
v7_new_cols = sorted(set(PHASE_MATRICES["v7e"][0].columns) - set(PHASE_MATRICES["v6"][0].columns))
prevalence_rows = []
for col in v7_new_cols:
    train_values = PHASE_MATRICES["v7e"][0][col]
    test_values = PHASE_MATRICES["v7e"][1][col]
    prevalence_rows.append(
        {
            "feature": col,
            "train_nonmissing_pct": float(train_values.notna().mean() * 100),
            "test_nonmissing_pct": float(test_values.notna().mean() * 100),
            "train_mean_or_prevalence": float(train_values.fillna(0).mean()),
            "test_mean_or_prevalence": float(test_values.fillna(0).mean()),
        }
    )
parser_prevalence = pd.DataFrame(prevalence_rows)
parser_prevalence.to_csv(OUTPUT_DIR / "v7_parser_prevalence_train_test_diagnostics.csv", index=False)


group_splits = make_group_splits(groups_center, n_splits=5)
phase_results = {}
phase_group_summaries = []
phase_ensemble_summaries = []
foldwise_model_rows = []
all_delta_rows = []

for phase in PHASE_ORDER:
    print(f"\n================ V7 phase comparison: {phase} ================")
    X_train_raw, X_test_raw = PHASE_MATRICES[phase]
    assert "CENTER" not in X_train_raw.columns
    assert "CENTER" not in X_test_raw.columns
    group_results = run_guarded_cv(
        models,
        group_splits,
        label=f"{phase} GroupKFold by CENTER selection",
        X_test_for_prediction=X_test_raw,
    )
    group_summary, ensemble_summary, selected = evaluate_phase_ensembles(phase, group_results)
    display(group_summary)
    display(ensemble_summary[["phase", "candidate", "group_oof_ipcw", "weights"]])
    phase_results[phase] = {
    ...
    phase_group_summaries.append(group_summary)
    phase_ensemble_summaries.append(ensemble_summary.drop(columns=["ensemble_oof"]))
...
v6_selected = phase_results["v6"]["selected"]
for phase in PHASE_ORDER[1:]:
    selected = phase_results[phase]["selected"]
    all_delta_rows.extend(
        phase_delta_rows(
            phase,
            selected["ensemble_oof"],
            v6_selected["ensemble_oof"],
            group_splits,
        )
    )
...
for phase in PHASE_ORDER:
    ...
    if phase == "v6":
        comparison_rows.append(
            {
                ...
                "v6_selected_group_oof_ipcw": selected["group_oof_ipcw"],
                "delta_vs_v6": 0.0,
        ...
        comparison_rows.append(acceptance_stats(phase, selected, v6_selected, delta_df))

phase_comparison = pd.DataFrame(comparison_rows)
display(phase_comparison)
pd.concat(phase_group_summaries, ignore_index=True).to_csv(OUTPUT_DIR / "v7_model_groupkfold_summary.csv", index=False)
pd.concat(phase_ensemble_summaries, ignore_index=True).to_csv(OUTPUT_DIR / "v7_ensemble_groupkfold_summary.csv", index=False)
pd.DataFrame(foldwise_model_rows).to_csv(OUTPUT_DIR / "v7_foldwise_model_scores.csv", index=False)
delta_df.to_csv(OUTPUT_DIR / "v7_foldwise_center_deltas.csv", index=False)
phase_comparison.to_csv(OUTPUT_DIR / "v7_phase_comparison_groupkfold.csv", index=False)


# %% [markdown]
# # Step 15: Save submissions only for best accepted V7 phase

# %%
accepted = phase_comparison[phase_comparison["accepted"] == True].copy()
if accepted.empty:
    no_accept_path = OUTPUT_DIR / "v7_no_phase_accepted_summary.csv"
    phase_comparison.to_csv(no_accept_path, index=False)
    print("No V7 phase accepted. Keeping V6 recommendation unchanged.")
    print("Saved:", no_accept_path)
else:
    accepted = accepted.sort_values(["selected_group_oof_ipcw", "phase"], ascending=[False, True])
    best_phase = accepted.iloc[0]["phase"]
    best_selected = phase_results[best_phase]["selected"]
    best_results = phase_results[best_phase]["group_results"]
    X_train_raw, X_test_raw = PHASE_MATRICES[best_phase]

    if not SAVE_V7_SUBMISSIONS:
        no_submission_path = OUTPUT_DIR / "v7_best_accepted_no_submission_summary.csv"
        phase_comparison.to_csv(no_submission_path, index=False)
        print("Best accepted phase:", best_phase)
        print("Selected candidate:", best_selected["candidate"])
        print("Weights:", best_selected["weights"])
        print("SAVE_V7_SUBMISSIONS=0: skipping final full-train fits and V7 submission CSV generation.")
        print("Saved diagnostic summary:", no_submission_path)
        sys.exit(0)

    selected_model_names = best_selected["model_names"]
    selected_weights = best_selected["weights"]
    groupfold_test_ranks = {
        model_name: best_results[model_name]["test_rank"]
    ...
    for model_name in selected_model_names:
        try:
            print(f"\nFinal full-train fit for {best_phase}/{model_name}...")
    ...
    final_groupfold_only = np.zeros(len(X_test_raw), dtype=float)
    for model_name in selected_model_names:
        final_groupfold_only += selected_weights[model_name] * groupfold_test_ranks[model_name]

    final_70_30 = np.zeros(len(X_test_raw), dtype=float)
    ...
    submission_main = pd.DataFrame({"ID": random_submission["ID"].values, "risk_score": final_70_30})
    submission_backup = pd.DataFrame({"ID": random_submission["ID"].values, "risk_score": final_groupfold_only})
    validate_submission_frame(submission_main)
    validate_submission_frame(submission_backup)

    main_path = OUTPUT_DIR / "submission_v7_best_accepted_70_30.csv"
    backup_path = OUTPUT_DIR / "submission_v7_best_accepted_groupfold_only_backup.csv"
    submission_main.to_csv(main_path, index=False)
    submission_backup.to_csv(backup_path, index=False)
    print("Best accepted phase:", best_phase)
    print("Selected candidate:", best_selected["candidate"])
